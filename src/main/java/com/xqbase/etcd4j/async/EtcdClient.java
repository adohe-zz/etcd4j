package com.xqbase.etcd4j.async;
...
public class EtcdClient {
    ...
    private static CloseableHttpAsyncClient buildHttpClient() {
        ...
        httpAsyncClient = HttpAsyncClients.custom().setDefaultRequestConfig(config).build();
     ...
     */
    public CEtcdResult set(String key, String value) throws CEtcdClientException {
    ...
    private ListenableFuture<CEtcdResult> asyncExecute(HttpUriRequest request, int[] expectedHttpStatusCodes, final int... excptedErrorCodes) {
        ListenableFuture<JsonResponse> response = asyncExecuteJson(request, expectedHttpStatusCodes);
        return Futures.transform(response, new AsyncFunction<JsonResponse, CEtcdResult>() {
            @Override
            public ListenableFuture<CEtcdResult> apply(JsonResponse jsonResponse) throws Exception {
                CEtcdResult result = jsonToCEtcdResult(jsonResponse, excptedErrorCodes);
    ...
    private ListenableFuture<JsonResponse> asyncExecuteJson(final HttpUriRequest request, final int[] expectedHttpStatusCodes) {
        ListenableFuture<RequestResponsePair> response = asyncExecuteHttp(request);

        return Futures.transform(response, new AsyncFunction<RequestResponsePair, JsonResponse>() {
            @Override
            public ListenableFuture<JsonResponse> apply(final RequestResponsePair pair) throws Exception {

                // Workaround for 307 response
                if (pair.response.getStatusLine().getStatusCode() == 307) {
                    Header location = pair.response.getFirstHeader("location");
                    ...
                    if (request instanceof HttpPut) {
                        ...
                        redirectRequest = new HttpPost(location.getValue());
                        ((HttpPost) redirectRequest).setEntity(((HttpPost) request).getEntity());
                    } else {
                ...
                // Workaround for 500 response
                if (pair.response.getStatusLine().getStatusCode() == 500) {
                    int depth = pair.depth.decrementAndGet();
                    if (depth > 0) {
                        return asyncExecuteJson(pair.request, expectedHttpStatusCodes);
                    }
                }

                JsonResponse jsonResponse = extractJsonResponse(pair.response, expectedHttpStatusCodes);
                return Futures.immediateFuture(jsonResponse);
            }
        });
    }

    private ListenableFuture<RequestResponsePair> asyncExecuteHttp(final HttpUriRequest request) {
        final SettableFuture<RequestResponsePair> future = SettableFuture.create();

        httpClient.execute(request, new FutureCallback<HttpResponse>() {
            @Override
            public void completed(HttpResponse httpResponse) {
                RequestResponsePair pair = new RequestResponsePair(request, httpResponse);
                future.set(pair);
            }
    ...
    static class RequestResponsePair {
        ...
        final AtomicInteger depth;

        public RequestResponsePair(HttpUriRequest request, HttpResponse response) {
            this.request = request;
            this.response = response;
            this.depth = new AtomicInteger(3);
        }
    }
    ...
    private CEtcdResult jsonToCEtcdResult(JsonResponse jsonResponse, int... exceptedErrorCodes) throws EtcdClientException {
        ...
        CEtcdResult result = parseCEtcdResult(jsonResponse.json);
